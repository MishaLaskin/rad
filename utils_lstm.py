...
class ReplayBuffer(Dataset):
    ...
    def __init__(self, obs_len, obs_shape, action_shape, capacity, batch_size, device,image_size=84, 
        ...
        self.obses = np.empty((capacity, obs_len, *obs_shape), dtype=obs_dtype)
        self.actions = np.empty((capacity, obs_len, *action_shape), dtype=np.float32)
    ...
    def add(self, obses, actions, reward, next_obs, done):
        np.copyto(self.obses[self.idx], obses)
